...
const ETHUSD_FEED = "0x59102b37de83bdda9f38ac8254e596f0d9ac61d2035c07936675e87342817160" as const;
const USDCUSD_FEED = "0x7416a56f222e196d0487dce8a1a8003936862e7a15092a91898d69fa8bce290c" as const;

// Verify against ethers.id (keccak256 of "ETHUSD" / "USDCUSD")
expect(ethers.id("ETHUSD")).to.equal(ETHUSD_FEED);
expect(ethers.id("USDCUSD")).to.equal(USDCUSD_FEED);

const WETH_PRICE_18DEC = 3_000n * 10n ** 18n; // $3000 in Stork 18-dec quantized format
...
async function deployFixture() {
  ...
  await storkAdapter.setFeedId(await weth.getAddress(), ETHUSD_FEED);
  ...
  await mockStork.setValue(ETHUSD_FEED, WETH_PRICE_18DEC, nowNs * 1_000_000_000n);
  ...
  return { veil, weth, usdc, mockStork, storkAdapter, owner, user, liquidator };
}
...
async function seedLiquidity(veil: VeilLend, weth: TokenMock, usdc: TokenMock6, liquidator: { address: string }, amount: bigint) {
  ...
  const seedState = makeInitialState({
    positionId: seedId, collateralAsset: BigInt(await weth.getAddress()),
...
async function createAndDeposit(veil: VeilLend, weth: TokenMock, usdc: TokenMock6, user: { address: string }, amount: bigint) {
  const id = (await veil.nextPositionId()) + 1n;
  const currentIndex = await veil.currentDebtIndex(await usdc.getAddress());
  const state = makeInitialState({
    positionId: id, collateralAsset: BigInt(await weth.getAddress()),
    debtAsset: BigInt(await usdc.getAddress()), currentIndex,
    controlSecret: BigInt(randHex()), salt: BigInt(randHex()),
  });
  await veil.createPosition(await weth.getAddress(), await usdc.getAddress(), bytes32(await computeCommitment(state)));
...
describe("Stork Oracle integration (production architecture)", () => {
  it("adapter returns the correct rescaled Stork price for WETH (18-dec)", async () => {
    const { storkAdapter, weth } = await loadFixture(deployFixture);
    const [price, updatedAt] = await storkAdapter.getPrice(await weth.getAddress());
  ...
  it("stale Stork update fails closed with StalePrice (via VeilLend)", async () => {
    const { veil, mockStork, weth } = await loadFixture(deployFixture);
    // Set the Stork timestamp to 2 hours ago (stale)
    const staleTs = (await chainSecs()) - 7200n;
    await mockStork.setValue(ETHUSD_FEED, WETH_PRICE_18DEC, staleTs * 1_000_000_000n);
    ...
    const [, staleUpdatedAt] = await adapter.getPrice(await weth.getAddress());
    expect(staleUpdatedAt).to.be.lessThan((await chainSecs()) - 3500n);
  });

  it("wrong feed mapping: zero feedId is rejected", async () => {
    const { storkAdapter, mockStork, weth } = await loadFixture(deployFixture);
    ...
    await expect(freshAdapter.setFeedId(await weth.getAddress(), ethers.ZeroHash)).to.be.reverted;
  });

  it("full borrow flow: WETH collateral → USDC debt via Stork prices (18→6 decimals)", async () => {
    const { veil, user, liquidator, weth, usdc } = await loadFixture(deployFixture);
    await seedLiquidity(veil, weth, usdc, liquidator, 40_000n * USD6);
    const { id, state } = await createAndDeposit(veil, weth, usdc, user, 10n * WAD);

    // Borrow 20,000 USDC ($20,000 ≤ $22,500 cap = 10 WETH × $3000 × 75%)
  ...
  it("same-tx atomicity: pushOracleUpdate + borrow via multicall (no separate refresh tx)", async () => {
    const { veil, user, liquidator, weth, usdc, mockStork } = await loadFixture(deployFixture);
    await seedLiquidity(veil, weth, usdc, liquidator, 40_000n * USD6);
    const { id, state } = await createAndDeposit(veil, weth, usdc, user, 10n * WAD);
    ...
    const [ethPrice] = await adapter.getPrice(await weth.getAddress());
    ...
    const [postPrice] = await adapter.getPrice(await weth.getAddress());
  ...
  it("borrow above the price-scaled dollar cap reverts with BorrowCapExceeded", async () => {
    const { veil, user, liquidator, weth, usdc } = await loadFixture(deployFixture);
    await seedLiquidity(veil, weth, usdc, liquidator, 40_000n * USD6);
    const { state } = await createAndDeposit(veil, weth, usdc, user, 10n * WAD);
  ...
  it("same oracle snapshot is used by ZK proof and VeilLend (price consistency)", async () => {
    const { veil, storkAdapter, weth, usdc } = await loadFixture(deployFixture);
    // Read the adapter price (what VeilLend will use on-chain)
    const [onChainWethPrice] = await storkAdapter.getPrice(await weth.getAddress());
    ...
    expect(onChainWethPrice).to.equal(3_000n * 10n ** 8n);
